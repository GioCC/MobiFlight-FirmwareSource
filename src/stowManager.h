...
public:
    ...
    StowManager(uint8_t *buffer, uint16_t maxSize)
        : buf(buffer), bufTop(buffer + maxSize), tail(buffer), curr(buffer), i_curr(0), i_count(0)
    {
    }
...
template <typename T>
T *StowManager::AddItem(T **itemPtr)
{
    ...
    if (in != nullptr) {
        new (reinterpret_cast<void *>(in)) T;
        // param init done later outside, by specialized functions
    }
    if (*itemPtr != nullptr) *itemPtr = reinterpret_cast<T *>(in);
